var Mtop = require("ui-ctool/src/c/mtop/index");
var Login = require('ui-ctool/src/c/login/index');
var Modal = require('ui-ctool/src/c/modal/index');
var Loading = require('ui-ctool/src/c/loading/index');
var Toast = Modal.toast;
...
module.exports = {
  ...
  /* //页面前缀http+host+port
   ...
   //千牛openwindow 不支持 noscheme 对于https 是否能支持需要验证
   return window.location.protocol + '//' + curHost + port + '/';
   },
   //html文件的路径
   preHtmlPath: function() {
   var path = '';
   if (isLocal) {
   path = 'htmls/';
   } else {
  ...
  getMtopData: function (param, suc_cb, fail_cb) {
    ...
    Mtop.request(params, function (result) {
      ...
      //登陆session 过期处理
      if (result && result.ret && result.ret[0] && result.ret[0].split("::")[0] == 'FAIL_SYS_SESSION_EXPIRED') {
      ...
      // JSTracker && JSTracker.error(result.api + '接口请求异常：' + JSON.stringify(result));
    });
  },
  //本地数据模拟
